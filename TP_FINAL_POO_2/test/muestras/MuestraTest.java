...
import muestras.estados.EstadoEnProceso;
...
class MuestraTest {
	...
	void setUp() throws Exception {
		...
		muestra = new Muestra("foto", ubicacion, usuario, TipoDeInsecto.VINCHUCA_GUASAYANA, gestor); // Muestra que sube
																										// el usuario
																										// (tipo de
																										// insecto
																										// arbitrario)
		...
		when(opinion.getEsUnExperto()).thenReturn(false); // La opinion de los que suben la muestra siempre es de
															// usuarios basicos
		...
		when(opinion4.getEsUnExperto()).thenReturn(true);
